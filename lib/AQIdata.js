...
const AQIdata = (useCurr, lat, long) => {
    ...
    const getLocation = () => {
        (async () => {
            ...
            getData(useCurr ? location.coords.latitude : lat, useCurr ? location.coords.longitude : long).then(response => {data = response[0];console.log("AQIdata.js | "+JSON.stringify(data));dataReady(data);});
        })();
    }
